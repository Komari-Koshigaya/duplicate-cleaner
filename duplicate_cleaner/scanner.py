...
class FileScanner:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        Args:
            max_workers: 最大线程数，None 则自动根据 CPU 核心数调整
        """
        # 线程数自适应：IO 密集型任务，使用 CPU 核心数的 2 倍
        self._max_workers = max_workers or min(os.cpu_count() * 2, 16)
        # 取消标志，线程安全
    ...
    def scan(
        ...
        quick_hash_groups = self._compute_quick_hashes(candidates, progress_callback)
        ...
        full_hash_groups = self._compute_full_hashes(full_hash_candidates, progress_callback)
    ...
    def _compute_full_hashes(
        ...
        with ThreadPoolExecutor(max_workers=min(self._max_workers, 4)) as executor:
